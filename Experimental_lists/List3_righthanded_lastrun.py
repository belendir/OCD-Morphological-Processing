...
expName = 'List3'  # from the Builder filename that created this script
expInfo = {
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/zeynepbelendir/Desktop/PILOT DATA/pilot experiments/List3_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    trials = data.TrialHandler(nReps=1.0, method='sequential', 
        ...
        trialList=data.importConditions('/Users/zeynepbelendir/Desktop/List3.xlsx'),
    ...
    for thisTrial in trials:
        ...
        prime.setText(prime3
        )
        target.setText(target3
        )
        key_resp.keys = []
        key_resp.rt = []
        _key_resp_allKeys = []
        # keep track of which components have finished
        trialComponents = [mask, prime, target, key_resp]
        for thisComponent in trialComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresTh = None
        ...
        while continueRoutine:
            ...
            if prime.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if prime.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > prime.tStartRefresh + 0.05-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if target.status == NOT_STARTED and tThisFlip >= 1.05-frameTolerance:
                # keep track of start time/frame for later
        ...
        if key_resp.keys != None:  # we had a response
            ...
            correctWorldList = list(thisTrial.values())


            correctword = correctWorldList[3]
            ...
            iscorrect = key_resp.keys == correctkey
            iscorrectnumb = 0
            if(iscorrect):
                iscorrectnumb = 1

            thisExp.addData('trials.iscorrect', iscorrectnumb)
